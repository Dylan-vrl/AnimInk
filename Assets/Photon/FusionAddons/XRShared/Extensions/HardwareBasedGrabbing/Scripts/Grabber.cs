...
namespace Fusion.XR.Shared.Grabbing {
    ...
    public class Grabber : MonoBehaviour
    {
        ...
        {
            hand = GetComponentInParent<IHardwareGrabber>();
            if (hand == null) Debug.LogError("Grabber should be placed next to an hardware hand or pen");
        }

        protected virtual bool IsGrabbing => hand is { isGrabbing: true };

        private void OnTriggerStay(Collider other)
        {
            //if (this.gameObject.CompareTag("Pen"))
                //Debug.Log("### TRIGGER STAYY " + other.gameObject.name + " " + other.enabled + " " + other.transform.parent.gameObject.name);
